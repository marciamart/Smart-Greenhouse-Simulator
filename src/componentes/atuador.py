...
class Atuador:
    ...
    def conectarGerenciador(self, host='localhost', port=5000):
        ...
        try:
            ...
            mensagem_inicial = {"autor": "Atuador :{self.tipo}", "id": self.id, "codigo_conexao":  self.codConex}
    ...
    def processarComando(self, comando, client_socket):
        ...
        estado = {"autor": "Atuador :{self.tipo}", "id": self.id, "status": self.status} # Enviar resposta ao gerenciador confirmando a ação tomada
